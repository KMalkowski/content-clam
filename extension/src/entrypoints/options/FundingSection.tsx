...
function SyncChoice({ status, busy, run }: { status: Status; busy: boolean; run: (label: string, work: () => Promise<unknown>) => Promise<void> }) {
  ...
  return (
    <div className="card stack">
      {unasked ? (
        <p>
          <strong>Choose which settings to keep.</strong> Your account may already hold settings from another device. Settings on this device are not synced
          until you pick one.
        </p>
      ) : (
      ...
      <div className="row">
        <button
          ...
          disabled={busy}
          onClick={() => run("Loaded settings from your account.", () => sendToBackground({ type: "syncFromServer" }))}
        >
        ...
        <button
          className={unasked ? "primary" : undefined}
          disabled={busy}
          onClick={() => run("Saved this device's settings to your account.", () => sendToBackground({ type: "pushSettingsToServer" }))}
        >
          Upload these settings
        </button>
      </div>
    </div>
  );
...
export function FundingSection({ status, refreshStatus, refreshSettings }: Props) {
  ...
  return (
    <section>
      ...
      <p className="muted">Each video analysis costs one credit or one request on your own key. You pick the source. Content Clam never switches on its own.</p>
      <div className="stack">
        ...
        <label className="card row" style={{ alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p className="muted">
              Requests go straight from this browser to api.typesafe.ai and are billed to your TypeSafe account. The key is stored only on this device, in
              extension storage, which is not an encrypted vault.
            </p>
            {status.fundingMode === "personal-key" && (
              <div className="row">
                <input
                  type="password"
                  placeholder={status.hasPersonalKey ? "Key saved. Paste a new one to replace it." : "ts_..."}
                  value={key}
                  onChange={(e) => setKey(e.target.value)}
                />
                <button onClick={saveKey} disabled={!key.trim() && !status.hasPersonalKey}>
        ...
        <label className="card row" style={{ alignItems: "flex-start", opacity: status.hostedAvailable ? 1 : 0.5 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p className="muted">
              Requests go to the Content Clam service, which forwards video metadata to TypeSafe and keeps no copy. Your first 100 analyses are free after email
              verification. Settings sync across devices.
            </p>
              ...
              <div className="stack">
                {status.signedIn ? (
                  <>
                    ...
                    <div className="row">
                      <a href={`${env.webUrl}/account`} target="_blank" rel="noreferrer">
                        <button className="primary">Buy credits</button>
                      </a>
                    </div>
                    <SyncChoice status={status} busy={busy} run={run} />
                  </>
                ) : (
                  <button className="primary" onClick={() => sendToBackground({ type: "openSignIn" })}>
